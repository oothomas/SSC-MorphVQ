...
class DiffusionNetMGPU(nn.Module):
    def __init__(
            self,
            C_in,
            C_out,
        ...
        super(DiffusionNetMGPU, self).__init__()
        ...
        # First affine layers
        self.first_lin = nn.Linear(C_in, C_width).to('cuda:0')
        self.block_num = 0

        # DiffusionNet blocks
        self.blocks1 = []
        for i_block in range(self.N_block // 4):
            ...
            self.add_module("block_" + str(self.block_num), self.blocks1[-1])
        ...
        for i_block in range(self.N_block // 4):
            ...
            self.add_module("block_" + str(self.block_num), self.blocks2[-1])
        ...
        for i_block in range(self.N_block // 4):
            ...
            self.add_module("block_" + str(self.block_num), self.blocks3[-1])
        ...
        for i_block in range(self.N_block // 4):
            ...
            self.add_module("block_" + str(self.block_num), self.blocks4[-1])

        # Last affine layers
        self.last_lin = nn.Linear(C_width, C_out).to('cuda:3')
    ...
    def forward(
        ...
        x, mass, L, evals, evecs, gradX, gradY = (x.to('cuda:0'), mass.to('cuda:0'),
                                                  L.to('cuda:0'), evals.to('cuda:0'),
                                                  evecs.to('cuda:0'), gradX.to('cuda:0'),
                                                  gradY.to('cuda:0'))
        for b in self.blocks1:
            x = b(x, mass, L, evals, evecs, gradX, gradY)

        x, mass, L, evals, evecs, gradX, gradY = (x.to('cuda:1'), mass.to('cuda:1'),
                                                  L.to('cuda:1'), evals.to('cuda:1'),
                                                  evecs.to('cuda:1'), gradX.to('cuda:1'),
                                                  gradY.to('cuda:1'))
        for b in self.blocks2:
            x = b(x, mass, L, evals, evecs, gradX, gradY)

        x, mass, L, evals, evecs, gradX, gradY = (x.to('cuda:2'), mass.to('cuda:2'),
                                                  L.to('cuda:2'), evals.to('cuda:2'),
                                                  evecs.to('cuda:2'), gradX.to('cuda:2'),
                                                  gradY.to('cuda:2'))

        for b in self.blocks3:
            x = b(x, mass, L, evals, evecs, gradX, gradY)

        x, mass, L, evals, evecs, gradX, gradY = (x.to('cuda:3'), mass.to('cuda:3'),
                                                  L.to('cuda:3'), evals.to('cuda:3'),
                                                  evecs.to('cuda:3'), gradX.to('cuda:3'),
                                                  gradY.to('cuda:3'))

        for b in self.blocks4:
            x = b(x, mass, L, evals, evecs, gradX, gradY)
...
class DiffusionNet(nn.Module):
    def __init__(
        ...
        for i_block in range(self.N_block):
            ...
            self.add_module("block_" + str(self.block_num), self.blocks[-1])
    ...
    def forward(
        ...
        # Apply each of the blocks
        x, mass, L, evals, evecs, gradX, gradY = (x, mass,
                                                  L, evals,
                                                  evecs, gradX,
                                                  gradY)
