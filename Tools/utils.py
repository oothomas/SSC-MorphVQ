...
def fMap2pMap(L1, L2, fmap):
    ...
    B2 = L2[:,:dim2]
    C12 = fmap
    _, T21 = spatial.cKDTree(B1@C12.T).query(B2)
    return T21
...
def bij_fMap2pMap(B1, B2, C12, C21):
    _, T12 = spatial.cKDTree(B2@C21.T).query(B1)
    _, T21 = spatial.cKDTree(B1@C12.T).query(B2)
    ...
    _, T12 = spatial.cKDTree(B2_aug).query(B1_aug)
    #
    B2_aug = np.concatenate([B1@C12.T, B1@C21], axis=1)
    B1_aug = np.concatenate([B2, B2], axis=1)
    _, T21 = spatial.cKDTree(B2_aug).query(B1_aug)
    return T12, T21
####
...
def QMap2pMap(B1, B2, Q12):
    ...
    B2_ = to_rl(B2[:, :k])
    #
    _, T21 = spatial.cKDTree(B1_Q12).query(B2_)
    return T21
